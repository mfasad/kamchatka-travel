...
document.querySelectorAll('[data-helicopter-quiz]').forEach((quiz) => {
  ...
  const updateQuiz = () => {
    ...
    if (focus === 'lake') { title = 'Смотрите удалённые природные маршруты с понятными правилами допуска.'; text = 'Для таких программ особенно важны сезон, разрешения, безопасность, питание и порядок действий при закрытии района.'; }
    if (focus === 'volcano') { title = 'Смотрите вулканические маршруты с прозрачной авиационной логистикой.'; text = 'Уточните маршрут полёта, возможность посадки, минимальную видимость, ограничения по багажу и наземную замену.'; }
...
document.querySelectorAll('[data-budget-quiz]').forEach((quiz) => {
  const result = quiz.querySelector('[data-budget-quiz-result]');
  const resultTitle = result?.querySelector('strong');
  const resultText = result?.querySelector('span');
  if (!result || !resultTitle || !resultText) return;

  const updateQuiz = () => {
    const format = quiz.querySelector('input[name="budget-format"]:checked')?.value || 'independent';
    const save = quiz.querySelector('input[name="budget-save"]:checked')?.value || 'comfort';
    let title = 'Сравните самостоятельную базу с однодневными выездами.';
    let text = 'Проверьте трансферы, питание, снаряжение и запасной день: именно они часто меняют итоговую стоимость.';

    if (format === 'tour') {
      title = 'Сравните многодневные туры по полной смете, а не по цене в заголовке.';
      text = 'Смотрите, что включено: проживание, питание, трансферы, входные сборы, запасной сценарий и доплаты за комфорт.';
    }

    if (format === 'camp') {
      title = 'Проверьте лагерь или волонтёрство как отдельный формат, а не замену туру.';
      text = 'Уточните дорогу до базы, обязанности, питание, страховку, снаряжение и свободные дни на вулканы или экскурсии.';
    }

    if (save === 'count') text = `${text} Если экономите на количестве выездов, выберите один главный маршрут и не ставьте его в последний день.`;
    if (save === 'dates') text = `${text} Если даты гибкие, сравните соседние заезды и условия переноса до оплаты.`;
